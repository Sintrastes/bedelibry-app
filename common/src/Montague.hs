...
getParseFromLexicon (SomeLexicon _ aT semantics) f input = f <$> getParse semantics aT input

-- | Get all parses of a string with regard to a lexicon.
getAllParsesFromLexicon :: SomeLexicon -> (forall a t. (Show a, Show t) => AnnotatedTerm a t -> r) -> String -> [r]
getAllParsesFromLexicon (SomeLexicon _ aT semantics) f input = f <$> getAllParses semantics aT input